...
int main() {
    ...
    // Imprimir los elementos impares de la matriz
      printf("Los numeros impares de la matriz que ingreso son:\n");
    for (int i = 0; i < filas; ++i) { // Recorre sobre cada fila de la matriz
        ...
        for (int j = 0; j < columnas; ++j) { // Recorre sobre cada columna de la fila actual
            if (matriz[i][j] % 2 != 0) {  // Verifica si el elemento en dicha posicion es par
                printf(" %d ", matriz[i][j]); // Imprime los numeros impares de la matriz ingresada
            } else{
                printf(" - "); // Imprimir un guion si el número no es impar
            }
        }
